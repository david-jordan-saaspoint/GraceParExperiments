...
class WelcomeController < ApplicationController
 ...
 #      session[:sfdc_pw] = "Saaspoint12VmnWWoESfynbRW4RqG754SXD"
  ...
  def populate_from_describe
     ...
     @sfdctables = Sfdctable.where(" fieldname not in (select sfdcfield from selectedfields where orgid= '#{@orgId}') and orgid = '#{@orgId}' " ).order("fieldname")
  end
  ...
  def fieldlist
     ...
     # @orgId= session[:orgId]
      ...
      @parlist = Array.new
      @par = Partable.where(" fieldname not in (select parfield from selectedfields where orgid= '#{@orgId}') and orgid = '#{@orgId}' " ).order("fieldname")
      @par.each do |paritem|
